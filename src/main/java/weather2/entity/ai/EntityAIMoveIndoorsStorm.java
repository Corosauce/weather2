...
public class EntityAIMoveIndoorsStorm extends EntityAIBase implements ITaskInitializer
{
    ...
    public boolean shouldExecute()
    {
        ...
        if (ClientTickHandler.weatherManager == null) return false;

        BlockPos blockpos = new BlockPos(this.entityObj);
        Vec3 pos = new Vec3(blockpos);

        boolean runInside = false;
        StormObject so = ClientTickHandler.weatherManager.getClosestStorm(pos, ConfigMisc.Villager_MoveInsideForStorms_Dist, StormObject.STATE_THUNDER);
