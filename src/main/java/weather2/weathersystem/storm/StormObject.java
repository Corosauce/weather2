...
import weather2.WeatherEntityTypes;
...
public class StormObject extends WeatherObject {
    ...
    public int updateLCG = RandomSource.create().nextInt();
        ...
        if (levelCurIntensityStage >= STATE_THUNDER && !isBaby() && !isPet()) {
            if (rand.nextInt((int) Math.max(1, ConfigStorm.Storm_LightningStrikeBaseValueOddsTo1 - (levelCurIntensityStage * 10))) == 0) {
                ...
                if (world.isLoaded(new BlockPos(x, 0, z))) {
                    ...
                    LightningBoltWeatherNew ent = new LightningBoltWeatherNew(WeatherEntityTypes.LIGHTNING_BOLT.get(), world);
    ...
    public void tickProgression() {
        ...
        if (world.getGameTime() % ConfigStorm.Storm_AllTypes_TickRateDelay == 0) {
            ...
            if (!performBuildup && !isPrecipitating() && rand.nextInt(randomChanceOfWaterBuildFromWater) == 0) {
                ...
                if (!performBuildup && (isInOcean || bgb.is(Tags.Biomes.IS_SWAMP) ||
                        bgb.is(BiomeTags.IS_JUNGLE) || bgb.is(BiomeTags.IS_RIVER))) {
                    performBuildup = true;
                }
            }
            ...
            if (isRealStorm()) {
                ...
                if (!hasStormPeaked) {
                    ...
                    if (levelCurIntensityStage < maxIntensityStage && (!ConfigTornado.Storm_NoTornadoesOrCyclones || levelCurIntensityStage < STATE_FORMING - 1)) {
    ...
    public void spinEntity(Object entity1) {
        ...
        if (entity1 instanceof Player) {
            ...
            if (ent.getDeltaMovement().y > 0.3F)
                ent.setDeltaMovement(ent.getDeltaMovement().x, 0.3F, ent.getDeltaMovement().z);
